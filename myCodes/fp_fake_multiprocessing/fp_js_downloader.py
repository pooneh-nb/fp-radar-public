    ...
    while start_date <= end_date:
        ...
        try:
            ...
            if response.status_code == 400:
                #9555
                utilities.append_file('/home/pooneh/Desktop/OpenWPM/jsons/error/fp_err_logs.json',
                                      script_url + str(response.status_code))
                break
            if response.content.decode()[0] != '{':
                utilities.append_file('/home/pooneh/Desktop/OpenWPM/jsons/error/fp_err_logs.json',
                                      script_url + "call back error")
                break
            if not response.ok:
            ...
            print("OOps: !", inst)
            utilities.append_file('/home/pooneh/Desktop/OpenWPM/jsons/error/fp_err_logs.json', script_url +
...
def create_fp_queued_files(unprocessed_fp_script_url_list, js_write_directory):
    ...
    for unprocessed_key, unprocessed_value in all_unprocessed_fp_script_url_list.items():
        if str(unprocessed_value['url_id']) not in processed_fp_script_url_list:
            remaining_unprocessed_fp_script_url.append(
                str(unprocessed_value['url_id']) + "|" + unprocessed_value['script_url'])

    for urlANDid in remaining_unprocessed_fp_script_url:
        start = time.time()
...
def main():
    with open("/home/pooneh/Desktop/OpenWPM/jsons/third_parties/all_3rdparty_fp_script_urls.json") as p:
    ...
    create_fp_queued_files(unprocessed_fp_script_url_list, js_write_directory)
